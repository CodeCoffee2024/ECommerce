...
{
    internal sealed class UpdateToEnableUnitOfMeasurementCommandHandler : ICommandHandler<UpdateToEnableUnitOfMeasurementCommand>
    {
        #region Fields
        ...
        public UpdateToEnableUnitOfMeasurementCommandHandler(
        ...
        public async Task<Result> Handle(UpdateToEnableUnitOfMeasurementCommand request, CancellationToken cancellationToken)
        {
            ...
            if (unitOfMeasurement.Status != Status.Disabled.GetDescription())
                return Result.Failure<Result>(Error.Concurrency);
            unitOfMeasurement.ToggleStatus(Status.Active.GetDescription());
